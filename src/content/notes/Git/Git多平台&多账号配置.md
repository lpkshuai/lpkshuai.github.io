---
slug: git-multi-platform-account
title: Git多平台/多账号配置
category: Git
type: setup
description: Git多平台/多账号配置。
status: published
tags: Git, SSH
...
- 该目录中一般有一对以 `id_dsa` 或 `id_rsa` 命名的文件，其中一个带有 `.pub` 扩展名。 `.pub` 文件是你的公钥，另一个则是与之对应的私钥。 如果找不到这样的文件（或者根本没有 `.ssh` 目录），你可以通过运行 `ssh-keygen` 程序来创建它们。
...
![image](/notes/git/git-multi-account-ssh-key-init.png)
...
- **New SSH key，输入一个标题，把本地的公钥 `.pub` 文件内容全选复制粘贴到文本框里**
  ![image](/notes/git/git-multi-account-ssh-key-config.png)

# 3. 配置config文件

- 在 `~/.ssh` 目录下，如果没有config文件可以通过 `touch ~/.ssh/config` 指令创建config文件，如果有则直接编辑这个文件：
...
> **其他参数配置详见** [点击这里](http://man.openbsd.org/OpenBSD-current/man5/ssh_config.5 "点击这里")
...
![image](/notes/git/git-multi-account-ssh-key-test.png)

**至此，配置完毕，可以提交代码到不同平台了** :sunglasses:
