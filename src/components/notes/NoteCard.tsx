      ...
      */}
      ...
      >
        ...
        <div className="grow">
          {/* 根据 featured 放大标题 */}
          <h3
            className={`line-clamp-2 font-bold text-(--foreground) transition-colors group-hover:text-(--accent) ${featured ? "text-2xl md:text-3xl" : "text-xl"}`}
          >
            {note.title}
          </h3>
          ...
          <p
            className={`mt-3 font-sans text-sm leading-relaxed text-(--foreground-muted) transition-colors group-hover:text-(--foreground) ${featured ? "line-clamp-4" : "line-clamp-3"}`}
          >
            {note.description}
          </p>
        </div>
        ...
        {/* 底部：元数据与标签 */}
        <div className="mt-6 flex flex-col gap-3 border-t border-dashed border-(--panel-border) pt-4">
          <div className="flex flex-wrap items-center gap-4 font-mono text-[11px] uppercase tracking-widest text-(--foreground-dim)">
          ...
          <div className="mt-1 flex flex-wrap gap-2">
            {note.tags.map((tag) => (
              <span
                key={tag}
                className="font-mono text-[10px] text-(--foreground-muted) transition-colors before:mr-0.5 before:opacity-50 before:content-['#'] group-hover:text-(--accent-strong)"
              >
                {tag}
              </span>
            ))}
